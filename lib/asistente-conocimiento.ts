...
1. "Teoría" — el contenido, partido en LECCIONES. A la izquierda hay una lista con todas (por ejemplo, en Shopify: "¿Qué es Shopify?", "Creación de cuenta", "Dominio", "Logo", "Dropify", "Releasit", "Trustoo", "Diseño de la tienda", "Políticas", "Desarrollo de producto") y arriba el porcentaje del módulo. Se abre una, se lee, y se pulsa "Marcar como vista" o "Siguiente" — la barra sube sola. El mismo porcentaje se ve en la tarjeta del módulo en la lista.
   ...
   Dentro de una lección puede haber un VIDEO DE APOYO de otro creador (YouTube), marcado como tal. No es la clase de Juan: son videos que explican bien ese punto concreto mientras él graba las suyas.
2. "Video" — la clase grabada. Se marca sola como vista al llegar al final: debajo hay una barra que muestra cuánto lleva. Solo en algún video viejo aparece el botón "Marcar video como visto" a mano.
   OJO: las clases de Juan se están GRABANDO y llegan el 15 de septiembre de 2026. Hasta entonces, la pestaña "Video" de los módulos que no tengan uno cargado dice "Juan está grabando esta clase". Eso es NORMAL, no es un fallo ni un bloqueo por su progreso, y no hay nada que pueda hacer para adelantarlo.
   NO hace falta esperar: el módulo entero está explicado en la Teoría, lección por lección y con gráficas, y varias lecciones traen un video de apoyo de otro creador dentro. La Práctica y el Test tampoco dependen de la clase de Juan. Si alguien pregunta por el video, dile eso y mándalo a la Teoría del módulo en el que va.
